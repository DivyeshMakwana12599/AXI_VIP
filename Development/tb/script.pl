...
#system("dve -full64 &")
system("vcs -sverilog -full64 -debug_access+r ei_axi4_top.sv && ./simv -gui +ei_axi4_SANITY_TEST +total_num_trans=4 +length=7 +size=3 +addr_type=0 +burst_type=2");
# system("./simv -gui +ei_axi4_SANITY_TEST +total_num_trans=4 +length=4 +size=3 +addr_type=0 +burst_type=0");
